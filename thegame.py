...
def game_loop(max):
    ...
    while run:
        ...
        if fail is True:
            ...
            text4 = textfont.render("ESC", 1, (255, 255, 255))
            win.blit(text4, (50 , 50))
            ...
            elif userInput[pygame.K_ESCAPE]:
                return max
...
while totalrun:
    ...
    key2 = loadify('key2.png')
    key2 = pygame.transform.scale(key2, (295*r, 100*r))
    esc_key1 = loadify('esc1.png')
    esc_key1 = pygame.transform.scale(esc_key1, (95*r, 95*r))
    esc_key2 = loadify('esc2.png')
    esc_key2 = pygame.transform.scale(esc_key2, (95*r, 100*r))
    cursor = loadify('cursor.png')
    cursor = pygame.transform.scale(cursor, (10*r, 15*r))
    bg = loadify('bg.png')
    bg = pygame.transform.scale(bg, (display_width, display_height))
    title = loadify('title.png')
    title = pygame.transform.scale(title, (354*r, 106*r))
    name = loadify('name.png')
    name = pygame.transform.scale(name, (193*r, 32*r))
    mx, my = pygame.mouse.get_pos()
    ticks+=1
    win.fill((255,255,255))
    win.blit(bg, (0, 0))
    win.blit(title, ((display_width - 354*r)/2 , 120*r))
    win.blit(name, (display_width - 200*r , display_height - 50*r))
    high_score = textfont.render(str(maxy), 1, (100, 100, 100))
    def digits(x):
        count_of_digits = 0
        while x > 0:
            x = int(x/10)
    ...
    digits_num = digits(maxy)
    win.blit(high_score, (display_width - (1 + digits_num)*30, 60))


    if resolution == 2:
        if mx > display_width/2 - 70*r and mx < display_width/2 + 70*r and my > display_height - 100*r and my < display_height - 70*r:
            text_screen = textfont.render("Window", 1, (100, 100, 100))
            win.blit(text_screen, (display_width/2 - 70*r, display_height - 100*r))
            mouse_presses = pygame.mouse.get_pressed()
            if mouse_presses[0]:
                resolution = 1
        else: 
            text_screen = textfont.render("Window", 1, (255, 255, 255))
            win.blit(text_screen, (display_width/2 - 70*r, display_height - 100*r))
    
    else:
        if mx > display_width/2 - 90*r and mx < display_width/2 + 85*r and my > display_height - 100*r and my < display_height - 70*r:
            text_screen = textfont.render("Fullscreen", 1, (100, 100, 100))
            win.blit(text_screen, (display_width/2 - 90*r, display_height - 100*r))
            mouse_presses = pygame.mouse.get_pressed()
            if mouse_presses[0]:
                resolution = 2
        else: 
            text_screen = textfont.render("Fullscreen", 1, (255, 255, 255))
            win.blit(text_screen, (display_width/2 - 90*r, display_height - 100*r))
    



    if mx > display_width/2 - 50*r and mx < display_width/2 + 50*r and my > display_height/2 and my < display_height/2 + 30*r:
        text_start = textfont.render("Start", 1, (100, 100, 100))
        win.blit(text_start, (display_width/2 - 50*r, display_height/2))
        mouse_presses = pygame.mouse.get_pressed()
        if mouse_presses[0]:
        ...
        text_start = textfont.render("Start", 1, (255, 255, 255))
        win.blit(text_start, (display_width/2 - 50*r, display_height/2))
    #win.blit(text_size, (display_width/2 -  , 150*r))

    if mx < 30 + 95*r and mx > 30 and my < 30 + 95*r and my > 30:
        win.blit(esc_key1, (30, 30))
        mouse_presses = pygame.mouse.get_pressed()
        if mouse_presses[0]:
            pygame.QUIT()
